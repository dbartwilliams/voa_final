...
import useImageKit from "../util/useImageKit";
...
const ArticleDetailPage = () => {
    ...
    const { data: postsData } = useQuery({
      ...
      // Post image (type "post")
     const imageSrc = useImageKit(data?.photo, "post");

       // Fallback for post image
     const fallbackPostSrc = useImageKit(null, "post");
  
    // // Always call the hook at the top level
    // const imageSrc = useImageKit(data?.photo || ""); // pass empty string until data loads
    // const fallbackSrc = useImageKit(); // fallback for default-post.png
    ...
    return (
      <MainLayout>
        ...
        <MetaTags 
            ...
            image={imageSrc || fallbackPostSrc}  // only the URL, no <img>
            slug={`blog/${data?.slug}`}
        />
          ...
          <BreadCrumbs data={breadCrumbsData} />
            ...
            <img
              className="rounded-xl w-[700px] transition-transform duration-500 ease-in-out transform hover:scale-105"
                src={imageSrc || fallbackPostSrc}
                alt={data.title}
                loading="lazy"
                onError={(e) => {
                e.currentTarget.src = fallbackPostSrc;
              }}
            />
